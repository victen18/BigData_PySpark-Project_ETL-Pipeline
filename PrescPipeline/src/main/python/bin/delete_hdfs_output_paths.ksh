...
if [ $status == 0 ]
  then
  printf "The HDFS output directory $CITY_PATH is available. Proceed to delete."
  hdfs dfs -rm -r -f $CITY_PATH
  printf "The HDFS Output directory $CITY_PATH is deleted before extraction !!! \n\n"
fi
...
if [ $status == 0 ]
  then
  printf "The HDFS output directory $FACT_PATH is available. Proceed to delete."
  hdfs dfs -rm -r -f $FACT_PATH
  printf "The HDFS Output directory $FACT_PATH is deleted before extraction !!! \n\n"
fi
...
HIVE_FACT_PATH=/user/hive/warehouse/prescpipeline.db/df_presc_final
